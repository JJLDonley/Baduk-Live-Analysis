...
export class PatternMatchingService {
  ...
  public nameMove(request: PatternMatchRequest): PatternMatchResponse {
    try {
      ...
      // Get move name
      const moveName = this.boardmatcher.nameMove(
        boardData,
        sign,
        vertex,
        options,
      ...
      if (moveName) {
        const patternResult = this.boardmatcher.findPatternInMove(
          boardData,
          sign,
          vertex,
          options,
    ...
    try {
      ...
      for (const pattern of options.library) {
        if (pattern.type === "corner") {
          ...
          for (
            const match of this.boardmatcher.matchCorner(boardData, pattern)
          ) {
            patterns.push({
              pattern: pattern,
              match: match,
          ...
          for (const anchor of pattern.anchors || []) {
            ...
            for (
              const match of this.boardmatcher.matchShape(boardData, [
                anchorX,
                anchorY,
              ], pattern)
            ) {
