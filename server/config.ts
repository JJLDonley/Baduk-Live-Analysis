...
export class ConfigManager {
  ...
  private getDefaultConfig(): ServerConfig {
    const exec = Deno.build.os == "linux" ? "./katago" : "./katago.exe";
    return {
      port: 8081,
      host: "0.0.0.0",
      katago: {
  ...
  private mergeConfigs(
    ...
    return {
      ...
      katago: {
        directory: loadedConfig.katago?.directory ||
          defaultConfig.katago.directory,
        executable: loadedConfig.katago?.executable ||
          defaultConfig.katago.executable,
        model: loadedConfig.katago?.model || defaultConfig.katago.model,
        config: loadedConfig.katago?.config || defaultConfig.katago.config,
      },
      analysis: {
        maxVisits: loadedConfig.analysis?.maxVisits ||
          defaultConfig.analysis.maxVisits,
        maxConcurrentAnalyses: loadedConfig.analysis?.maxConcurrentAnalyses ||
          defaultConfig.analysis.maxConcurrentAnalyses,
        timeoutMs: loadedConfig.analysis?.timeoutMs ||
          defaultConfig.analysis.timeoutMs,
        includePolicy: loadedConfig.analysis?.includePolicy !== undefined
          ? loadedConfig.analysis.includePolicy
          : defaultConfig.analysis.includePolicy,
        includeOwnership: loadedConfig.analysis?.includeOwnership !== undefined
          ? loadedConfig.analysis.includeOwnership
          : defaultConfig.analysis.includeOwnership,
      },
      logging: {
  ...
  public getKataGoExecutablePath(): string {
    ...
    return resolve(
      join(
        Deno.cwd(),
        this.config.katago.directory,
        this.config.katago.executable,
      ),
    );
  }
  ...
  public getKataGoModelPath(): string {
    return resolve(
      join(Deno.cwd(), this.config.katago.directory, this.config.katago.model),
    );
  }

  public getKataGoConfigPath(): string {
    return resolve(
      join(Deno.cwd(), this.config.katago.directory, this.config.katago.config),
    );
  }
